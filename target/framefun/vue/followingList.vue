<template>
  <div class="follow-block" style="text-align: center">
    <div style="height: 50px;text-align: left"><div style="color:#606266;display: inline-block"><b>{{artist.user_name}}的关注</b></div><el-tag style="margin-left: 5px"><i class="el-icon-star-on"></i>{{count}}</el-tag></div>
...
module.exports={
  ...
  methods:{
    pageChange(e){
      var _this = this;
      this.$router.push({path:`/follow/following/${this.userId}/${e-1}`});
    },
    follow(e){
      var _this = this;
      var us = this.users[e];
  ...
  mounted(){
    ...
    axios({
      ...
      var data = response.data;
      _this.artist = data;
      _this.count = parseInt(_this.artist.user_following);
      _this.pageCount = Math.floor((parseInt(_this.artist.user_following)+23)/24);
    ...
    axios({
      method: 'get',
      url: `/follow/following/${this.userId}/${this.currentPage-1}`
    }).then(function (response) {
      var data = response.data;
      for (var i in data) {
